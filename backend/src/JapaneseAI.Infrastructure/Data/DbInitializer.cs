    ...
    public static class DbInitializer
    {
        public static async Task InitializeAsync(AppDbContext context, ILogger logger, string? dbProvider = null)
        {
            try
            {
                ...
                else if (!await context.KanjiDictionaries.AnyAsync())
                {
                    logger.LogInformation("No KANJIDIC2 file found. Seeding KanjiDictionary sample data...");
                    context.KanjiDictionaries.AddRange(new List<KanjiDictionary>
                    {
                ...
                {
                    logger.LogInformation("Database already seeded with vocabulary.");
                    return;
                }
